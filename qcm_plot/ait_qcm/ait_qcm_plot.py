...
ait_filename = '01ait -0.4V PtQCM in 4.5g KNO3.txt'
qcm_filename = '01qcm Au cap parafilm electodes.dta'
output_filename = 'ait_qcm.png' 
# Specify an AIT start time since we typically wait a while for the QCM to
# stabilize before measuring data:
ait_start_time = 60.0 #s
...
ait_data = pandas.read_csv(ait_filename, skiprows=16, header=0, names=('time',
                            'current', 'charge'))
...
qcm_data = qcm_data[qcm_data.t >= ait_start_time].reset_index(drop = True)
...
axes[0].set_ylim(0e-5, 1.0e-5)
axes[0].plot(ait_data['time'], ait_data['current'], color = 'red')
